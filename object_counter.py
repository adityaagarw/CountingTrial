...
class ObjectCounter:
    ...
    def extract_and_process_tracks(self, im, tracks):
        ...
        for box, track_id, cls in zip(boxes, track_ids, clss):
            ...
            if self.counting_region.contains(Point(track_line[-1])):
                ...
                dwell_time = time.time() - self.entry_time[track_id]

                # Display the dwell time
                dwell_time_label = f"Dwell Time (ID {track_id}): {dwell_time:.2f} seconds"
                cv2.putText(im, dwell_time_label, (int(box[0]), int(box[1]-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
            else:
            ...
            # If the object is not in the counting region, remove it from the entry time dictionary
            else:
                if track_id in self.entry_time:
                    del self.entry_time[track_id]
    ...
    def start_counting(self, im0, tracks):
        ...
        if tracks[0].boxes.id is None:
            if self.view_img:
                self.display_frames()
            return im0
        self.extract_and_process_tracks(im0, tracks)
